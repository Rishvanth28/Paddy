...
def payment(request):
    ...
    return render(request, 'orders_app/payment.html', context)
...
def approve_cash_payment(request, request_id):
    """API endpoint to approve/reject cash payment requests"""
    try:
        data = json.loads(request.body)
        approved = data.get('approved')  # Boolean
        notes = data.get('notes', '')
        
        # Get the cash payment request
        cash_request = get_object_or_404(CashPaymentRequest, request_id=request_id)
        
        if cash_request.status != 0:  # Not pending
            return JsonResponse({
                'success': False,
                'message': 'This request has already been processed'
            })
        
        # Get the user who's processing this request
        processor_id = request.session.get('user_id')
        processor = get_object_or_404(AdminTable, admin_id=processor_id)
        
        # Update the request
        cash_request.status = 1 if approved else 2  # 1 = Approved, 2 = Rejected
        cash_request.processed_by = processor
        ...
        if approved:
            # If approved, update the order payment
            order = cash_request.order
            current_paid_amount = order.paid_amount or 0
            new_paid_amount = current_paid_amount + cash_request.amount
            order.paid_amount = new_paid_amount
            ...
            Payments.objects.create(
                ...
                date=timezone.now().date(),
                reference=f"cash_payment_request_{request_id}",
                proof_link=cash_request.reference,
                proof_ref=cash_request.reference
            )
        
        return JsonResponse({
            'success': True,
            'message': f'Cash payment request {"approved" if approved else "rejected"} successfully'
        })
